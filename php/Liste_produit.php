            ...
            if(isset($_SESSION['admin'])){
                ...
                foreach($dbh->query("SELECT * FROM Alizon.Produit WHERE id_vendeur='".$_SESSION["id_vendeur"]."' ORDER BY id_produit", PDO::FETCH_ASSOC) as $row) {
                    ...
                    if($row['quantite_stock'] <= 10) {
                        echo '<div class=alert_prd id ="article" class="col-6 col-sm-6 col-md-6 col-lg-3 col-xl-3" ><button id="btn" name="ID" type="submit" form="Detail" value="'.$row['id_produit'].'" class="h-100 btn btn-outline-primary" style="border: 5px solid red;background-color : lightcoral;"><img id ="images" src="'.$nom_dossier.$chaine[0].'" class="rounded img-fluid"> <p>'.$row['libelle'].'</p> <p>/!\ En stock : ' . $row['quantite_stock'] . ' /!\</p> <p id="prix"> '.$row['prix_ttc'].'€</p></button></div>';
                    } else {
                        echo '<div class=all_prd id ="article" class="col-6 col-sm-6 col-md-6 col-lg-3 col-xl-3" ><button id="btn" name="ID" type="submit" form="Detail" value="'.$row['id_produit'].'" class="h-100 btn btn-outline-primary"><img id ="images" src="'.$nom_dossier.$chaine[0].'" class="rounded img-fluid"> <p>'.$row['libelle'].'</p> <p id="prix"> '.$row['prix_ttc'].'€</p></button></div>';
                    }
