                        ...
                        <div class="row justify-content-center mt-3">
                            ...
                            <?php
                                ...
                                try
                                {
                                    ...
                                    if (isset($_GET['tel'])) {
                                        $tel= $_GET['tel'];
                                        $stmt = $dbh->prepare("SELECT mail FROM alizon._client WHERE tel = ?");
                                        $stmt->execute([$tel]);
                                        
                                        $res = $stmt->fetch(PDO::FETCH_ASSOC);
                                        
                                            $subject = '/!\ Identifiant Alizon /!\ ';
                                            $message = "Bonjour, ceci est un message qui vous indique que ce mail correspond à votre identifiant pour le site Alizon.\nVotre identifiant est : ".$res['mail']."\nCordialement, l'équipe Alizon.";
                                            $headers = 'Content-Type: text/plain; charset="UTF-8"';
                                            $stmt2 = $dbh->prepare("SELECT count(*) FROM alizon._client WHERE tel = ?");
                                            $stmt2->execute([$_GET['tel']]);
                                            $res1 = $stmt2->fetch();
                                            $res1 = $res1['count'];
                                            if ($res1 != 0){
                                                foreach( $res as $row){
                                        ...
                                        echo "Si votre numéro de téléphone était associé à un de nos compte client, nous vous avons envoyé un mail. Pensez à regarder vos mails.";
                                    }
