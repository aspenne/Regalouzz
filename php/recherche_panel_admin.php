...
<?php 
    ...
    if(isset($_POST['Info'])){
        $recherche = $_POST['Info'];
        $recherche = trim($recherche);
        $separateur = "#[ ]+#"; // 1 ou plusieurs espaces
        $mots = preg_split($separateur, $recherche);
        $i  = 0;
        foreach($mots as $mot){
            if($i == 0){
                $requete= "SELECT * FROM alizon._client where nom like '%$mot%' or prenom like '%$mot%' or mail like '%$mot%' or tel like '%$mot%' or CAST(id_client AS TEXT) like '%$mot%'";
            }
            else{
                $requete = "SELECT * FROM (".$requete.") as t where nom like '%$mot%' or prenom like '%$mot%' or mail like '%$mot%' or tel like '%$mot%' or CAST(id_client AS TEXT) like '%$mot%'";
            }
            $i++;
        }
        if(isset($requete)){
            $requete = $requete." order by id_client asc";
            $lst_client = $dbh->query($requete);
        }
        else{
            headers("location:panel_admin.php");
        }
    }
    //Zone de recherche
    ?>
